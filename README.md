...
```
## **Blast**
1. We need to run BLAST to compare healthy vs breast cancer samples. This can show up similarities or lacktherof
2. NOTE TO SELF:  MAKE A FILE CALLED healthypatients with the healthy data. the query file is Cancer.fasta and i think i need to change the name.
2. load blast and create a nucleotide database
```
    -- module load BLAST \
    -- makeblastdb -in healthypatients.fna -dbtype nuvl -out healthysamples_db \
```
3. Run BLASTN against the database of healthy patients
```
    -- blastn -query cancer.fasta -db healthysamples_db -out results_nucl.txt -outfmt "6 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore stitle" 
```
4. Find no hit sequences
```
    -- grep "^>" cancer.fasta | sed 's/>//' > all_ids.txt
    -- sort all_ids.txt > all_sorted.txt
    -- sort hits.txt > hits_sorted.txt
    -- comm -23 all_sorted.txt hits_sorted.txt > no_hits_ids.txt
```
Push to the repository and open in excel for visualization
the .txt has IDs of unique sequences
